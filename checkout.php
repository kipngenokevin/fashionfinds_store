...
<body>
    ...
    <section class="checkout-details-section">
        <div class="container-checkout">
            <div class="row checkout-details-section">
                
                <div class="col-sm-12 col-md-6 half-container">
                <h2>Fill in your details</h2>
                    <form class="customer-details-form" action="submit-cart.php" method="POST">
                        <label class="customerform-label" for="name">Name</label>
                        <input type="text" id="name" name="name" required><br><br>

                        <label class="customerform-label" for="email">Email</label>
                        <input type="email" id="email" name="email" required><br><br>

                        <label class="customerform-label" for="address">Address</label>
                        <input type="text" id="address" name="address" required><br><br>

                        <input type="hidden" name="total_price" value="<?php echo $total_price; ?>">

                        <?php foreach ($_SESSION["cart"] as $key => $item) { ?>
                            <input type="hidden" name="cart[<?php echo $key; ?>][name]" value="<?php echo $item["name"]; ?>">
                            <input type="hidden" name="cart[<?php echo $key; ?>][price]" value="<?php echo $item["price"]; ?>">
                            <input type="hidden" name="cart[<?php echo $key; ?>][quantity]" value="<?php echo $item["quantity"]; ?>">
                        <?php } ?>

                        <input class="checkout-section-button" type="submit" value="Submit">
                    </form>
                </div>
